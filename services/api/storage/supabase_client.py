"""
Thin wrapper sobre Supabase Storage.
Si las variables de entorno no están configuradas, las operaciones
se saltean silenciosamente (útil en desarrollo sin Supabase).
"""
import logging
import os
...
def upload_file(local_path: str, storage_path: str,
    ...
    """Sube un archivo local a Supabase y devuelve la URL pública. None si no hay config."""
...
def delete_file(storage_path: str) -> None:
    if not SUPABASE_URL or not SUPABASE_KEY:
        return
    try:
        _get_client().storage.from_(SUPABASE_BUCKET).remove([storage_path])
    except Exception as exc:
        logger.warning("Error borrando de Supabase (%s): %s", storage_path, exc)


def path_from_url(url: str) -> str | None:
    """Extrae el storage_path de una URL pública de Supabase."""
    if not url:
        return None
